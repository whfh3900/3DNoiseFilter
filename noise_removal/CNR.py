...
def parse_arguments():
    ...
    parser.add_argument('--outdir', type=str, default='', required=True, help='output folder (estimated point cloud properties)')
...
def get_meaned_displacements(dataset, moved_points, n_neighbours):
    ...
    nearest_neighbours = torch.LongTensor(shp.kdtree.query(shp.pts, n_neighbours)[1])
