...
def load_shape(point_filename, normals_filename, curv_filename, pidx_filename, clean_points_filename):
    ...
    #print("pts: ", end="")
    #print(pts)
    tmp_pointID, tmp_XYZ = np.hsplit(pts, [1])
    #sys.setrecursionlimit(int(max(1000, round(pts.shape[0]/10)))) # otherwise KDTree construction may run out of recursions
    #sys.setrecursionlimit(int(max(1000, round(tmp_XYZ.shape[0] / 10))))
    #sys.setrecursionlimit(int(max(1000, round(pts.shape[0] / 10))))
    sys.setrecursionlimit(int(max(1000, round(pts.shape[0] / 10))))
    #kdtree = spatial.cKDTree(pts, 10)
    #kdtree = spatial.cKDTree(tmp_XYZ, 10)
    kdtree = spatial.cKDTree(pts, 10)
    #kdtree = spatial.cKDTree(pts, 10)
    clean_points_kdtree = None

    #print("clean_points: ", end="")
    #print(clean_points)
    if clean_points is not None:
        clean_points_kdtree = spatial.cKDTree(clean_points, 10)
    #return Shape(pts=pts, kdtree=kdtree, normals=normals, curv=curvatures, pidx=patch_indices, clean_points = clean_points,
    #return Shape(pts=tmp_XYZ, kdtree=kdtree, normals=normals, curv=curvatures, pidx=patch_indices, clean_points=clean_points,
    return Shape(pts=pts, kdtree=kdtree, normals=normals, curv=curvatures, pidx=patch_indices, clean_points=clean_points,
        clean_kdtree = clean_points_kdtree)
...
class Shape():
    def __init__(self, pts, kdtree, normals=None, curv=None, pidx=None, clean_points=None, clean_kdtree=None):
        self.pts = pts
        self.kdtree = kdtree
...
class PointcloudPatchDataset(data.Dataset):
    ...
    def __init__(self, root, shapes_list_file, patch_radius, points_per_patch, patch_features,
                 seed=None, identical_epochs=False, use_pca=True, center='point', point_tuple=1, cache_capacity=1, point_count_std=0.0, sparse_patches=False, shape_names = None,
                 logfile=False):
        ...
        for shape_ind, shape_name in enumerate(self.shape_names):
            ...
            pointID_only = np.loadtxt(point_filename).astype('float64')

            # modify to add pointID info
            #print("pts: ", end="")
            #print(pts)
            tmp_pointID, tmp_XYZ = np.hsplit(pts, [1])
            tmp_pointID_only, tmp_XYZ_forbid = np.hsplit(pointID_only, [1])
            #print("tmp_XYZ: ", end="")
            #print(tmp_XYZ)

            #np.save(point_filename+'.npy', pts)
            np.save(point_filename + '.npy', tmp_XYZ)
            #np.save(point_filename + '_pointID' + '.npy', tmp_pointID)
            np.save(point_filename + '_pointID' + '.npy', tmp_pointID_only)
            ...
            if logfile:
                inputTimeSplit = divmod(time.time() - startTime, 60)
                logfile.write("Data input(shape: %s) duration: %d min %d sec\n" % (shape_name, inputTimeSplit[0], inputTimeSplit[1]))
    ...
    scale_ind_range, patch_pts_valid, patch_pts, clean_points=False):
        if clean_points:
            ...
            #tmp_pointID, tmp_XYZ = np.hsplit(shape.pts, [1])
            #print("shape.pts: ", end="")
            #print(shape.pts)
            #patch_point_inds = np.array(shape.kdtree.query_ball_point(tmp_XYZ[center_point_ind, :], patch_radius))
            patch_point_inds = np.array(shape.kdtree.query_ball_point(shape.pts[center_point_ind, :], patch_radius))
        ...
        if len(patch_point_inds)>0:# there can be no clean points nearby
            #patch_pts[start:end, :] = torch.from_numpy(points_base[patch_point_inds, :])
            #print("patch_point_inds: ", end="")
            #print(points_base[patch_point_inds, :])
            #patch_pts_PID_Included = points_base[patch_point_inds, :]
            #patch_pts_PID_Removed = np.array([np.delete(patch_pts_PID_Included, [0, 0])])
            #patch_pts[start:end, :] = torch.from_numpy(patch_pts_PID_Removed)
            #tmp_pointID, tmp_XYZ = np.hsplit(points_base[patch_point_inds, :], [1])
            #patch_pts[start:end, :] = torch.from_numpy(tmp_XYZ)
        ...
        if self.center == 'mean':
            ...
            #patch_pts_PID_Included = shape.pts[center_point_ind, :]
            #patch_pts_PID_Removed = np.array([np.delete(patch_pts_PID_Included, [0, 0])])
            #patch_pts[start:end, :] = patch_pts[start:end, :] - torch.from_numpy(patch_pts_PID_Removed)
            #tmp_pointID, tmp_XYZ = np.hsplit(points_base[patch_point_inds, :], [1])
            #patch_pts[start:end, :] = patch_pts[start:end, :] - torch.from_numpy(tmp_XYZ)
            patch_pts[start:end, :] = patch_pts[start:end, :] - torch.from_numpy(shape.pts[center_point_ind, :])
    ...
    def load_shape_by_index(self, shape_ind):
        ...
        return load_shape(point_filename, normals_filename, curv_filename, pidx_filename, clean_points_filename)
