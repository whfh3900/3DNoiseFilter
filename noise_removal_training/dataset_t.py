...
class PointcloudPatchDataset(data.Dataset):
    ...
    def __init__(self, root, shapes_list_file, patch_radius, points_per_patch, patch_features,
                 seed=None, identical_epochs=False, use_pca=True, center='point', point_tuple=1, cache_capacity=1, point_count_std=0.0, sparse_patches=False, shape_names = None, logfile=False):
    ...
    def load_shape_by_index(self, shape_ind):
        ...
        clean_points_filename = os.path.join(self.root, self.shape_names[shape_ind]+'.xyz') if self.include_clean_points else None
