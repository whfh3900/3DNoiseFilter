...
class STN(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.num_scales == 1:
            ...
            if x.is_cuda:
                #x_scales = Variable(torch.cuda.DoubleTensor(x.size(0), 1024*self.num_scales, 1))
                x_scales = Variable(torch.cuda.FloatTensor(x.size(0), 1024*self.num_scales, 1))

            else:
                #x_scales = Variable(torch.DoubleTensor(x.size(0), 1024*self.num_scales, 1))
        ...
        if not self.quaternion:
            iden = Variable(torch.from_numpy(np.identity(self.dim, 'float64')).clone()).view(1, self.dim*self.dim).repeat(batchsize, 1)
            ...
            #iden = Variable(torch.DoubleTensor([1, 0, 0, 0]))
            ...
            if x.is_cuda:
                #trans = Variable(torch.cuda.DoubleTensor(batchsize, 3, 3))
                trans = Variable(torch.cuda.FloatTensor(batchsize, 3, 3))

            else:
				#trans = Variable(torch.DoubleTensor(batchsize, 3, 3))
...
class PointNetfeat(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.num_scales == 1:
            ...
            if x.is_cuda:
                #x_scales = Variable(torch.cuda.DoubleTensor(x.size(0), 1024*self.num_scales**2, 1))
                x_scales = Variable(torch.cuda.FloatTensor(x.size(0), 1024*self.num_scales**2, 1))

            else:
                #x_scales = Variable(torch.DoubleTensor(x.size(0), 1024*self.num_scales**2, 1))
