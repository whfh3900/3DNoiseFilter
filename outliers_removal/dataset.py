...
class PointcloudPatchDataset(data.Dataset):
    ...
    def __init__(self, root, shapes_list_file, patch_radius, points_per_patch, patch_features,
        ...
        for shape_ind, shape_name in enumerate(self.shape_names):
            ...
            if self.include_outliers:
                ...
                #outliers = np.loadtxt(outliers_filename).astype('float64')
            ...
            if self.include_clean_points:
                ...
                #clean_points = np.loadtxt(clean_points_filename).astype('float64')
    ...
    scale_ind_range, patch_pts_valid, patch_pts, clean_points=False):
        ...
        #print(type(patch_point_inds))
        #print("points_base: ", end="")
    ...
    def __getitem__(self, index):
        ...
        #patch_pts = torch.DoubleTensor(self.points_per_patch*len(self.patch_radius_absolute[shape_ind]), 3).zero_()
        ...
        if self.include_clean_points:
            tmp = []
            #patch_clean_points = torch.DoubleTensor(self.points_per_patch, 3).zero_()
        ...
        if self.use_pca:
            ...
            #trans = torch.eye(3).double()


        # get point tuples from the current patch
        if self.point_tuple > 1:
            #patch_tuples = torch.DoubleTensor(self.points_per_patch*len(self.patch_radius_absolute[shape_ind]), 3*self.point_tuple).zero_()
